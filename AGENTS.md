...
Next: closed-loop v1 is complete (openPMD, uproot ROOT I/O, the Xsuite
adapter, mean-field space charge). Ahead: engine-track M1'
containerized CI and further recipes, the GPU lane (float32 backend
validation + fully labeled benchmarks on the WSL2 box), then the
