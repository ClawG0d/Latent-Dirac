...
- **安全范围钉死降为三处**（所有者决定）：`EXPECTED_EXCLUSIONS` 元组 +
  ...
  **不要把整节加回**
- **桌面客户端全部落地(勿重做)**:跨平台 Electron app,自然语言对话
  生成场景 → 本地引擎跑 → Daylight 四象限 dashboard(左上 ~70% 3D 视口 /
  右上实时物理 / 左下对话 / 右下 Ledger·Inspector·Sweep 可切换)。
  **模拟全在本地;AI 是 BYOK**——用户自己贴 Anthropic key,只存主进程
  (`safeStorage` 加密)、只发给 Anthropic,渲染进程永远拿不到。仓库侧:
  本地引擎 **stdio 桥**(`latent_dirac/bridge/`,`python -m latent_dirac.bridge`,
  先打印 `{"ready":true}` 再按行收发 JSON——**无 HTTP、无端口、无 FastAPI**;
  HTTP server 与 `[server]` extra 已退役)、离线 3D(plotly 内联)、
  Electron 外壳(`desktop/`)、PyInstaller + electron-builder 打包
  (`desktop/packaging/`)。**打包陷阱**:冻结引擎必须 **非 editable** 安装
  latent-dirac + spec 里 `collect_submodules("latent_dirac")`,否则冻结包
  运行时报 `No module named 'latent_dirac'`。`services/ai_gateway/` 保留为
  可选托管后备,非默认路径。核心逻辑(sidecar stdio JSON-RPC、生成→校验→
  运行纠错重试、面板数据变换、打包引擎路径解析)是注入式 Node,
  `node --test` 无网跑(见 `desktop/test/`)。**这台 Mac 有显示器、可本机
  验 GUI**(此前文档说的 "CPU-only 不能验 GUI" 是误读——CPU-only 只指无
  CUDA、不产性能数字)。已在本机出并验证 arm64 未签名 .dmg;出**签名**
  .dmg/.exe 仍需 owner 的证书 / Windows 机。设计记录见同日
  `desktop-electron-shell`(含 BYOK + stdio 附录)/ `desktop-packaging` spec。

- **buffer_gas_cooling 表化(T5,已落地)**:`buffer_gas_cooling` 新增
  **table-based 模式**。新包 `latent_dirac/collisions/`:provenance 校验的
  截面表 loader(`load_cross_sections`)+ null-collision(Skullerud)算子
  (`buffer_gas_collide`,指数间隔的 Poisson 候选碰撞,弹性/非弹性阈值损失
  下限 (3/2)kT/损失通道)。元件用 `cross_section_path` + `gas_pressure_pa`
  (n = P/(k_B T))走表化模式,或三个常数率字段走参数化模式(二选一,已校验)。
  报告含**截面 provenance 块**(截面版四元组)。**诚实性钉死**:随附的
  `examples/data/cross_sections/n2_positron_toy.csv` 是**明确标注的合成占位表**
  (fidelity `parameterized`,非物理数据),真实 DOI 截面表后补才升为
  `table-based`——**绝不编造截面值**。设计记录见同日 `buffer-gas-collisions`
  与 `buffer-gas-table-based-landing` spec。
...
- **T3 场图导入器**：CST "Export Plot Data (ASCII)" 导入器已落地
  ...
  已由 owner 决定暂缓**（2026-07-07）——精确格式只在 SIMION 应用内帮助、
  公开查不到，按诚实性纪律不照臆测格式写解析器；有样例 `.patxt` 或格式
  说明再恢复。
- 其余 Python 侧方向（排在 GPU 车道之后）：JAX 后端场图支持；
  物理填充方向：降能片物理（走 M3 产额表路线）。**buffer-gas 截面表
  基础设施已在 T5 落地**（见 §二），剩下的只是补一张**真实 DOI 引用的
  正电子截面表**(合成占位表在位、机制齐活)与算子分裂;真实数据到位后
  按三处状态规则把该表升为 `table-based`。
  ...
  及其可微期望存活项）；**交互式 3D 查看器**（2026-07-06 落地为
  `render_scene_animation` 播放/擦洗动画 + `latent-dirac render
  --animate`，已被桌面客户端 3D 面板复用）；**Surko buffer-gas 冷却
  参数化 + 表化两档**（`buffer_gas_cooling` 元素 + `latent_dirac/collisions/`，
  T5）；**桌面客户端全套**（见 §二，勿重做）。
