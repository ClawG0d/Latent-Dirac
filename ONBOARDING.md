...
- **桌面客户端全部落地(勿重做)**：跨平台 Electron app,自然语言对话
  生成场景 → 本地引擎跑 → 3D 面板。**模拟全在本地,只有 AI 调用出网。**
  仓库侧五块都在:本地引擎 HTTP API(`latent_dirac/server/`,`[server]`
  extra,`python -m latent_dirac.server` 打印 `PORT <n>`)、离线 3D
  (plotly 内联)、托管 AI 网关(`services/ai_gateway/`,引擎版本无关、
  key 只在服务端)、Electron 外壳(`desktop/`)、PyInstaller +
  electron-builder 打包配方(`desktop/packaging/`)。核心逻辑
  (sidecar 生命周期、网关→校验→运行的纠错重试、打包版引擎路径解析)
  是注入式 Node,`node --test` 无网跑(见 `desktop/test/`)。
  **本机(CPU-only Mac)只验逻辑测试 + 构建/lint;GUI 启动、出签名
  安装包(.dmg/.exe)在 owner 的 Mac/Windows 上做**——签名证书、
  Windows 出 .exe 的 CI 车道、网关部署与 Anthropic key 都归 owner 侧。
  设计记录见同日 `desktop-electron-shell` / `desktop-packaging` spec。
