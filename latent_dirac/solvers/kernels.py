...
def boris_step(
    ...
    gamma_minus = np.sqrt(1.0 + np.sum(u_minus * u_minus, axis=1))
    ...
    gamma_new = np.sqrt(1.0 + np.sum(u_new * u_new, axis=1))
    ...
    return (
        np.where(keep, position_new, position_m),
        np.where(keep, u_new, u),
        np.where(alive, time_new, time_s),
    )
